...
import { createThread, updateThread, sendForApproval } from "@/lib/apis/thread";
import { showMessage } from "@/components/ui/MessageBox";
import { ThreadDraftData, ThreadFormData } from "@/types/thread";
...
const ThreadForm = ({
  ...
  const [editorData, setEditorData] = useState<OutputData | null>( initialData.content ? JSON.parse(initialData.content as string) : null );

  const [threadTags, setThreadTags] = useState<Tag[]>(initialData.tags || []);
  ...
  const {
    ...
    defaultValues: {
      ...
      spaceId:
        initialData.spaceId ||
        (availableSpaces.length === 1 ? availableSpaces[0].spaceId : ""),
    },
  });
      ...
      if (mode === "create") {
        ...
        if (createResponse.success && createResponse.data) {
          ...
          if (submitResponse.success) {
            ...
            mapApiErrors(submitResponse.errors);
          }
        } else {
          mapApiErrors(createResponse.errors);
        }
      } else if (mode === "edit" && threadId) {
        const updateResponse = await updateThread({
          threadId,
          ...data,
          content: JSON.stringify(editorData),
        });
        ...
        if (updateResponse.success) {
          const submitResponse = await sendForApproval(threadId);
          if (submitResponse.success) {
            showMessage({
              type: "success",
              message: "Thread submitted for approval successfully!",
          ...
          mapApiErrors(updateResponse.errors);
        }
      }
    } catch (error) {
      showMessage({
