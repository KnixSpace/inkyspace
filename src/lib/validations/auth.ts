...
export const registerSchema = yup.object().shape({
    ...
    email: yup.string().max(255).email("Please enter a valid email address").required("Email is required"),
    ...
    role : yup.string().oneOf(["U","O"],"Invalid role selected").required("Role is required")
});
